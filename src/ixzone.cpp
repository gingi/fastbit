...
//
// (de-serialization) reconstruct zone from content of a file
...
// after the bit vectors of this level are written, the next level ibis::zone
...
ibis::zone::zone(const ibis::column* c, ibis::fileManager::storage* st,
	...
	array_t<int32_t>
	    offs(st,
		 8*((offset+sizeof(int32_t)*(nobs+1)+
		     2*sizeof(uint32_t)+7)/8)+sizeof(double)*nobs*3,
		 nobs+1);
#ifdef DEBUG
	if (ibis::gVerbose > 5) {
...
void ibis::zone::estimate(const ibis::qContinuousRange& expr,
	...
	if (sub.size() == nobs && sub[hit1] != 0) { // sub is defined
		...
		if (i >= sub[hit1]->nobs) { // impossible ?
		    ...
		    same = true;
		    if (i > 0) {
			sub[hit1]->addBits(0, i, lower, *(bits[hit1]));
		    }
		    upper.copy(lower);
		}
		    ...
		    same = true;
		    sub[hit1]->addBits(0, i+1, lower, *(bits[hit1]));
		    upper.copy(lower);
		}
	    }
	}
	...
	if (sub.size() == nobs && sub[cand0] != 0) { // sub defined
		...
		if (i >= sub[cand0]->nobs) { // impossible case
		    ...
		    sub[cand0]->addBits(i+1, sub[cand0]->nobs, lower,
					*(bits[cand0]));
		    upper.copy(lower);
		}
		    ...
		    sub[cand0]->addBits(i, sub[cand0]->nobs, lower,
					*(bits[cand0]));
		    upper.copy(lower);
		}
	    }
	...
	if (cand0 == 0) { // sub[0] never defined
		...
		if (i >= sub[cand0]->nobs) { // unlikely case
		    ...
		    if (bits[cand0] != 0) {
			ibis::bitvector tmp;
			sub[cand0]->addBits(i+1, sub[cand0]->nobs, tmp,
					    *(bits[cand0]));
			lower |= tmp;
			if (same) {
			    upper.copy(lower);
		    ...
		    if (bits[cand0] != 0) {
			ibis::bitvector tmp;
			sub[cand0]->addBits(i, sub[cand0]->nobs, tmp,
					    *(bits[cand0]));
			lower |= tmp;
			if (same) {
			    upper.copy(lower);

// File $Id: parth3dw.cpp,v 1.1 2009/04/01 03:27:42 kewu Exp $
...
ibis::part::fill3DBinsWeighted(const ibis::bitvector &mask,
...
ibis::part::fill3DBinsWeighted3(const ibis::bitvector &mask,
	...
	if (val3 == 0) return -8L;
	ierr = fill3DBinsWeighted(mask, val1, begin1, end1, stride1,
				  val2, begin2, end2, stride2,
				  *val3, begin3, end3, stride3,
				  wts, weights, bins);
	delete val3;
	break;}
    case ibis::UBYTE: {
	...
	LOGGER(ibis::gVerbose > 3)
	    << "Warning -- ibis::part::fill3DBinsWeighted3 -- unable to "
...
ibis::part::fill3DBinsWeighted2(const ibis::bitvector &mask,
				const array_t<T1> &val1,
				const double &begin1, const double &end1,
				const double &stride1,
	...
	if (val2 == 0) return -6L;
	ierr = fill3DBinsWeighted3(mask, val1, begin1, end1, stride1,
				   *val2, begin2, end2, stride2,
				   col3, begin3, end3, stride3,
				   wts, weights, bins);
	delete val2;
	break;}
    case ibis::UBYTE: {
    ...
    default: {
	LOGGER(ibis::gVerbose > 3)
	    << "Warning -- ibis::part::fill3DBinsWeighted2 -- unable to "
...
long ibis::part::get3DBins(const char *constraints, const char *cname1,
    ...
    array_t<double> *wts;
    if (mask.cnt() > (nEvents >> 4)) { // read all values, convert to double
	ibis::bitvector tmp;
	tmp.set(1, nEvents);
	wts = wcol->selectDoubles(tmp);
    }
    else { // read selected values, convert to double
	wts = wcol->selectDoubles(mask);
    }
    if (wts == 0) {
	LOGGER(ibis::gVerbose >= 0)
	    << "Warning -- ibis::part[" << (m_name ? m_name : "")
	    << "]::get3DBins failed retrieve values from column "
	...
	if (val1 == 0) return -4L;
	ierr = fill3DBinsWeighted2(mask, *val1, begin1, end1, stride1,

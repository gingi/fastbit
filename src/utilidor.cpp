...
uint32_t ibis::util::sortAll_split(array_t<T1>& arr1, array_t<T2>& arr2) {
	...
	lg.buffer() << (int)pivot1;
    lg.buffer() << ", ";
    if (sizeof(T2) > 1)
	lg.buffer() << pivot2;
    else
	lg.buffer() << (int)pivot2;
    lg.buffer() << ")";
	...
	if (sizeof(T1) > 1)
	    ...
	    lg.buffer() << (int) arr1[j0];
	lg.buffer() << ", ";
	if (sizeof(T2) > 1)
	    lg.buffer() << arr2[j0];
	else
	    lg.buffer() << (int)arr2[j0];
	lg.buffer() << ")";
    }
    if (j1 < i0)
...
void ibis::util::sort_quick3(array_t<T1>& keys, array_t<T2>& vals) {
    ...
    uint32_t iprt = ((nelm >> ibis::gVerbose) > 0 ?
		     (1 << ibis::gVerbose) : nelm);
    ibis::util::logger lg(4);
    lg.buffer() << "ibis::util::sort_quick3(keys[" << keys.size() << "], vals["
		<< vals.size() << "]) completed";
    if (sorted) {
	lg.buffer() << " successfully";
    }
...
void ibis::util::sort_heap(array_t<T1>& keys, array_t<T2>& vals) {
    ...
    uint32_t iprt = ((nelm >> ibis::gVerbose) > 0 ?
		     (1 << ibis::gVerbose) : nelm);
    ibis::util::logger lg(4);
    lg.buffer() << "ibis::util::sort_heap(keys[" << keys.size() << "], vals["
		<< vals.size() << "]) completed";
    if (sorted) {
	lg.buffer() << " successfully";
    }
...
void ibis::util::sort_partition3(array_t<T1>& keys, array_t<T2>& vals,
	...
	lg.buffer() << (int)*pivot;
    lg.buffer() << ", cnt1 = " << cnt1 << ") # elements = " << starteq << ": ";

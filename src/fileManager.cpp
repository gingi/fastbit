...
// Copyright 2000-2012 Univeristy of California
//
// Purpose:
...
int ibis::fileManager::getFile(const char* name, storage** st,
	...
	*st = (*it).second;
    ...
    if (ierr < 0) {
	LOGGER(ibis::gVerbose >= 0)
	    << evt << " -- unable to free up " << ibis::util::groupby1000(bytes)
...
ibis::fileManager::storage::storage(const char* begin, const char* end)
	...
	if (ierr < 0) {
	    LOGGER(ibis::gVerbose >= 0)
		<< "Error -- fileManager::storage is unable to find "
	...
	if (m_begin == 0) { // second failure
		...
		lg() << "Error -- fileManager copy constructor "
		    "is unable to allocate " << ibis::util::groupby1000(nbytes)
		     << " bytes\n";
